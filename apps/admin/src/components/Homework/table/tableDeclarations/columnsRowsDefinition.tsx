...
const columnsRowsDefinition: ColumnDefCustom<TableRowType>[] = [
  {
    ...
    cell: ({ getValue }) => {
      const { teacher } = getValue<{
        teacher: TableRowType['teacher'];
      }>();

      const teacherFullName = teacher ? `${teacher.firstName} ${teacher.lastName}` : 'No teacher';
      const teacherInitials = teacher ? `${teacher.firstName.charAt(0)}${teacher.lastName.charAt(0)}` : 'NT';

      return (
        <RowContainer className='w-72'>
          <div className='flex items-center gap-3'>
            <Avatar className='size-9'>
              <AvatarImage src={teacher?.avatar?.url} alt={teacherFullName} />
              <AvatarFallback className='text-xs font-medium uppercase'>{teacherInitials}</AvatarFallback>
            </Avatar>
            <span className='truncate font-medium whitespace-nowrap'>{teacherFullName}</span>
          </div>
        </RowContainer>
      );
    },
