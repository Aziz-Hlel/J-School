...
const TeacherExtraCurricularHeader = () => {
  const schoolId = useCurrentSchoolId();

  const navigate = useNavigate();
  const currentProfile = useGetCurrentProfile();

  const { data: teacherExtraCurricularData, isLoading } = useQuery({
    queryKey: ['teachers', currentProfile?.id, 'extra-curricular'],
    queryFn: async () => teacherService.getExtraCurricular(schoolId, currentProfile?.id || ''),
  });

  const teacherExtraCurricular = teacherExtraCurricularData?.data ?? [];

  const [currentExtraId, setCurrentExraId] = useState<string | null>();

  // Primary TanStack Query
  const { data } = useQuery({
    queryKey: ['extra-curriculars', currentExtraId],
    queryFn: () => extraCurricularService.get(schoolId, currentExtraId || ''),
    enabled: !!currentExtraId,
  });
    ...
    <Card className='border-border bg-card overflow-hidden shadow-sm'>
      <CardContent className='relative p-0'>
        ...
        <div className='relative flex flex-col gap-6 p-6 md:p-8 lg:flex-row lg:items-end lg:justify-between'>
          <div className='w-full min-w-0 space-y-4 lg:w-auto'>
            <Select
              onValueChange={(value) => {
          ...
          {extraCurricular && currentExtraId && (
            <div className='w-full lg:w-auto'>
              <Tabs defaultValue='overview' className='w-full lg:w-60'>
                <TabsList className='bg-muted grid h-11 w-full grid-cols-2 rounded-full p-1'>
                  <Link
                    to={`/extracurriculars/teacher/${currentExtraId}/overview`}
                  ...
                  <Link
                    to={`/extracurriculars/teacher/${currentExtraId}/feed`}
                    className='data-[state=active]:bg-background data-[state=active]:text-foreground rounded-full'
                  >
...
export default TeacherExtraCurricularHeader;
